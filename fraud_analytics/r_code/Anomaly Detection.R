...
# Load Needed Data #
ins <- read.csv(file = "fraud_analytics/data/aggregated_policy_nolabel.csv", header = TRUE)
...
plot(
  x = ins$Time_Between_CL_R, y = ins$Cov_Limit_Claim / 1000,
  xlim = c(-10, 30),
  ylim = c(-10, 3000),
...
rad <- sqrt(qchisq(0.9999975, ncol(df)))
ellipse(
  center = robustcenter, shape = robustcov,
  radius = rad, col = "red"
)
...
plot(CovLimit ~ Time,
  data = df, cex = sqrt(knn_score) + 0.01, pch = 20,
  xlim = c(-10, 30),
  ylim = c(-10, 3000),
...
hist(ins_cade$odds[which(ins_cade$prob > 0.01)],
  breaks = 50,
