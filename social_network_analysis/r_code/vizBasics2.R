...
plot(slack,
  edge.arrow.size = .3, vertex.label = NA, vertex.size = 10,
...
plot(slack,
  edge.arrow.size = .2, vertex.label = V(slack)$name, vertex.size = 10,
...
legend(
  x = -1.5, y = 0, unique(V(slack)$Cohort), pch = 21,
...
edges <- data.frame(get.edgelist(slack)) # data frame with edge list
colnames(edges) <- c("source", "target")
edges$source <- as.numeric(edges$source) - 1 # make the numbering start from 0!!
edges$target <- as.numeric(edges$target) - 1

V(slack)$ID <- as.numeric(as.factor(V(slack)$name)) - 1
nodes <- data.frame(vertex_attr(slack)) # data frame with node data
...
forceNetwork(
  ...
  charge = -100, fontSize = 12, opacity = 0.8, zoom = T, legend = T
)
